...
- **Environment Variables** (toutes configurables sans toucher `config.js`) :
  ...
  | `RENDER_URL` | **optionnel sur Render** — Render injecte `RENDER_EXTERNAL_URL` automatiquement, le bot le détecte. À définir uniquement sur Fly/Railway/etc. |
...
Le bot s'auto-ping déjà toutes les 5 min via `RENDER_URL` (ou `RENDER_EXTERNAL_URL` injecté par Render), mais Render Free dort si **aucun trafic externe** ne tape l'endpoint. Ajouter un monitor gratuit :
...
**Mise en place une seule fois — Environment Group partagé**
Render supporte les **Environment Groups** : un groupe de variables réutilisable sur N services. Crée-en un une fois pour ne plus jamais retaper tes credentials :
1. Render Dashboard → menu de gauche → **Env Groups** → **New Environment Group**
2. Nom : `dazbot-shared`
3. Variables (partagées par tous les amis) :
   - `SUPABASE_URL`, `SUPABASE_KEY` (TES credentials Supabase)
   - `GEMINI_API_KEY` (si tu partages ta clé Gemini)
   - `TZ` = `Africa/Porto-Novo`

**Workflow pour ajouter un ami (30 sec par ami, sans rien push) :**
1. Render → **New Web Service** pointant sur le même repo (jamais besoin de fork).
2. Onglet **Environment** → **Link Environment Group** → choisir `dazbot-shared` → toutes les vars partagées sont héritées automatiquement.
3. Ajouter les env vars **spécifiques à cet ami** (uniquement) :
   - `PHONE_NUMBER` = numéro de l'ami (ex: `22912345678`)
   - `OWNER_NUMBER` = numéro affiché comme propriétaire
   - `OWNER_NAME` = nom de l'ami
   - `BOT_ID` = identifiant unique (ex: `ami_jean`)
   - `USE_PAIRING_CODE` = `true` (recommandé pour Render)
4. Render déploie automatiquement. Logs → Pairing Code → tu le donnes à l'ami → il lie l'appareil dans WhatsApp.
5. Session sauvée dans Supabase sous `ami_jean:creds`, `ami_jean:pre-key-1`, etc.
6. UptimeRobot → 1 monitor par URL Render, ping 5 min.

> Note : `RENDER_URL` n'est pas dans la liste car Render injecte `RENDER_EXTERNAL_URL` automatiquement et le bot le détecte tout seul.

Comme tout est dans les env vars, **tu n'as jamais besoin de toucher `config.js` ni de push** pour ajouter un ami. Un seul repo, un Env Group partagé, N services Render avec leurs vars perso.
